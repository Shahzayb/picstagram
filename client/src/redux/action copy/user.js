...
import { toggleLike } from '../../api/photo';
...
export const toggleLikeUserPhoto = (photoId, isLiked, done) => async (
  dispatch
) => {
  try {
    if (isLiked) {
    ...
    done('', isLiked ? 'Unliked post!' : 'Liked post!');
  } catch (e) {
    console.log(e);
    if (isLiked) {
      dispatch({
    ...
    done(isLiked ? 'Failed to unlike post!' : 'Failed to like post!');
  }
};
